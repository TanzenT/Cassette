...
import tanzent.cassette.theme.TextInputLayoutUtil
...
import tanzent.cassette.util.Util.sendCMDLocalBroadcast
...
class Tag(context: Context, song: Song?) : ContextWrapper(context) {
  ...
  fun detail() {
    if (!tagEditor.initSuccess) {
      ToastUtil.show(App.getContext(), getString(R.string.init_failed))
      return
    }
